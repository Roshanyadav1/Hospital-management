            ...
            if request.data.get('hospital_email') == request.data.get('hospital_owner_email'):
                try:
                    error = Error.objects.get(
                        error_title='HOSPITAL_SAME_EMAIL')
                    response_message = error.error_message
                    response_code = error.error_code
                    Response.status_code = error.error_code
                except:
                ...
                hospital = Hospital.objects.get(
                    hospital_email=request.data.get('hospital_email'))
                ...
                try:
                    error = Error.objects.get(
                        error_title='REGISTRATION_SUCCESS')
                    response_message = error.error_message
                    response_code = error.error_code
                    Response.status_code = error.error_code
                except:
        ...
        if id is not None:
            if Hospital.objects.filter(hospital_id=id).count() >= 1:
                ...
                try:
                    ...
                    response_code = status.HTTP_400_BAD_REQUES
                return Response(
                    {
                        'status': response_code,
                        'message': response_message,
    ...
    def patch(self, request, input, format=None):
        ...
        if Hospital.objects.filter(hospital_id=id).count() >= 1:
            ...
            serializer = HospitalSerializer(
                hospital, data=request.data, partial=True)
            serializer.is_valid(raise_exception=True)
            serializer.save()
            response_message = ""
            response_code = ""
